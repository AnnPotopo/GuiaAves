...
import PrintEngine from './PrintEngine'; // IMPORTAMOS EL NUEVO MOTOR
...
export default function EditorLayout() {
    ...
    const [printSettings, setPrintSettings] = useState({
        showBleed: false,
        showMargins: false,
        splitPages: true, // La imprenta exige páginas sueltas
        cropMarks: true,
        slugInfo: true
    });
    ...
    return (
        <>
            {/* 1. EDITOR UI (SE OCULTA TOTALMENTE AL IMPRIMIR PARA NO ROMPER EL LAYOUT) */}
            <div className="flex h-screen bg-gray-200 overflow-hidden font-sans print:hidden">
                        ...
                        {activeTab === 'print' && sidebarOpen && (
                            <div className="px-4">
                                <div className="bg-emerald-900/40 border border-emerald-800 p-4 rounded-lg mb-4">
                                    ...
                                    <p className="text-[11px] text-emerald-100/70 text-justify leading-relaxed">Las fuentes quedarán incrustadas automáticamente. Tras generar el PDF aquí, usa un conversor (ej. Acrobat) para pasarlo a perfil CMYK (FOGRA39).</p>
                                </div>

                                <ControlPanel title="Ajustes de Pre-Prensa">
                                    ...
                                    <p className="text-[10px] text-gray-500 mt-2 text-justify">La imprenta requiere que cada archivo vaya en "Páginas Sueltas" para poder compaginar (la Tripa).</p>
                                </ControlPanel>
                                ...
                                <button onClick={handlePrint} className="w-full mt-6 bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded shadow-lg flex items-center justify-center gap-2">
                                    <Printer className="w-5 h-5" /> Preparar PDF (Tripa)
                                </button>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex-1 flex flex-col relative overflow-auto bg-gray-300 z-0">
                    <div className="min-h-full p-8 flex flex-col items-center justify-center">
                        <div className="transition-all duration-300 transform scale-100 origin-center shadow-2xl ring-1 ring-gray-400/50">
                            {/* Para visualizar en pantalla renderizamos sin activar isPrintMode */}
                            <PageRenderer pageData={pages[currentPageIndex]} bookSize={bookSize} printSettings={printSettings} isPrintMode={false} />
                        </div>
                        <p className="text-xs text-gray-500 mt-6 font-mono bg-white/50 px-3 py-1 rounded">Libro: {bookTitle} | Formato: {bookSize}</p>
                    </div>
                </div>
            ...
            {/* 2. MOTOR DE IMPRESIÓN (SOLO EXISTE Y ES VISIBLE CUANDO PRESIONAS CTRL+P / CMD+P) */}
