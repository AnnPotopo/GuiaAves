...
export default function UserProfile() {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            setLoading(true);
            try {
                // 1. Intentar obtener la portada y avatar del usuario
                ...
                // 2. Obtener SOLO las publicaciones PÚBLICAS de este autor
                const qBooks = query(
    ...
    if (loading) {
        return <div className="h-full flex items-center justify-center bg-gray-50"><Loader2 className="w-10 h-10 animate-spin text-emerald-600" /></div>;
    }
    ...
    return (
        <div className="h-full bg-gray-50 flex flex-col font-sans overflow-y-auto pb-20 relative">
            ...
            {/* Botón para regresar */}
            <button onClick={() => navigate(-1)} className="absolute top-6 left-6 z-20 bg-black/40 hover:bg-black/60 text-white p-2.5 rounded-full backdrop-blur-md transition shadow-lg">
            ...
            <div className="w-full shrink-0 bg-white border-b border-gray-200 pb-6">
                <div className="relative w-full h-48 md:h-64 bg-slate-800">
                    {displayCover ? (
                        <img src={displayCover} alt="Portada" className="w-full h-full object-cover opacity-70" />
                    ) : (
